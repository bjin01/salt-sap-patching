...
def run(groups=[], **kwargs):
    ...
    if len(systemid_list_all_groups) > 0:
        ...
        log.info("unique_ids: {}".format(unique_ids))
        ...
        for server in unique_systemid_list_all_groups:
            ...
            if server['name'] and server['name'] != "":
                #log.info("See: {}: {}".format(server['name'], server['id']))
                all_data.append({"name": server['name'], "id": server['id']})
                minionlist.append(server["name"])

        """ log.info("Run online minion check.")
        if not minionlist:
            log.info("No active minions in list. Exit.")
        return """
        
        
        if 'no_presence_check' in kwargs and kwargs['no_presence_check']:
            log.info("Skipping minion online presence check.")
            online_minions = minionlist
        else:
            log.info("Run online minion check.")
        ...
        """ if not online_minions:
            log.error("Failed to get online minions. Exit")
            return """
        
        #log.info("all_data: {}".format(all_data))
                ...
                if len(result_upg_pkgs) > 0:
                    ...
                    for pkg in result_upg_pkgs:
                        if pkg['to_package_id'] not in pkg_ids:
                            ...
                            log.debug("Not adding duplicate pkg id: {} {}".format(pkg['to_package_id'], pkg['name']))
                else:
                    log.info(f"{sname} does not have upgradable packages.")
                if len(pkg_ids) > 0:
        ...
        if len(systemlist_with_pkgids) > 0:
            ...
            #Apply highstate first
            log.debug("Add highstate job: {}".format("highstate"))
            sort_num = _get_rhnactionchainentry_sortorder(client, key, chain_actionID)
            for system in systemlist_with_pkgids:
                state_action_id = _addState(client, key, system["id"], label, chain_actionID, [], sort_num)
                if state_action_id == 0:
                    log.error("Failed to add apply highstate job to action chain for {}. Exit.".format("highstate"))
                    return
                else:
...
def _scheduleApplyStates(client, key, servername, sid, states):
    earliest_occurrence = six.moves.xmlrpc_client.DateTime(datetime.now() + timedelta(minutes=60))
    if len(states) > 0:
        try:
            poststate_jobid = client.system.scheduleApplyStates(key, sid, sid, states, earliest_occurrence, 0)
            return poststate_jobid
        except Exception as exc:  # pylint: disable=broad-except
            err_msg = 'Exception raised when trying to schedule post state for {} {}'.format(servername, exc)
            log.error(err_msg)
            return 0
    return 0
    ...
    try:
        ...
        with _get_cursor() as curs:
            ...
            if select_rhnactionchainentry_result:
                log.debug("select_rhnactionchainentry_result[0] sort_order: {} is instance {}".format(select_rhnactionchainentry_result[0], type(select_rhnactionchainentry_result[0])))
...
def _addState(client, key, sid, label, chain_id, states, sort_num):
    ...
    if states_string != "":
        ...
        states_string_jobname = f'"Apply highstate"'
    ...
    create_rhnactionapplystates = """INSERT INTO rhnactionapplystates (id, action_id, states, test) VALUES(nextval('rhn_act_apply_states_id_seq'), %s, NULLIF(%s, ''), 'N') RETURNING id;"""
