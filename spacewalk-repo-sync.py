...
def read_exclude_channels_file():
    if os.path.exist(exclude_channels):
        with open(exclude_channels, 'r') as myfile:
            l_exclude_channels = yaml.safe_load(myfile)
        log("following channels will not be synced: {}", format(l_exclude_channels))
    else:
        log("No exclude.list file found")
...
def read_exclude_channels_file():
    if os.path.exists(exclude_channels):
        with open(exclude_channels, 'r') as myfile:
            l_exclude_channels = yaml.safe_load(myfile)
        #log(0, "following channels will not be synced: %s" % l_exclude_channels)
        return l_exclude_channels
    else:
        log(0, "No exclude.list file found")
    return None
...
def main():
    ...
    exclude_channels_list = read_exclude_channels_file()
    if isinstance(exclude_channels_list, dict):
        if len(exclude_channels_list["exclude_channels"]) > 0:
            for exclude_ch in exclude_channels_list["exclude_channels"]:
                for ch in list(d_ch_repo_sync):
                    if exclude_ch in ch:
                        del d_ch_repo_sync[ch]
                        log(0, "channel %s is excluded from repo-sync." % exclude_ch)
